...
table = TEMPLATE[
    TEMPLATE.find('{',  TEMPLATE.find('CNO_HUFFMAN_TABLE')) + 1
  : TEMPLATE.find('};', TEMPLATE.find('CNO_HUFFMAN_TABLE'))
]

xs = eval('[' + table.replace('{', '(').replace('}', ')') + ']')
# Left-align and sort by Huffman code so that characters with same bits 0..i
# form a continuous range.
xs = sorted((code << 32 >> sz, sz, i) for i, (code, sz) in enumerate(xs))
