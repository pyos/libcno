...
static void log_frame(int fd, cno_frame_t *frame, int recv)
{
...
static void log_message(int fd, cno_message_t *msg, int recv)
{
    const char *e = recv ? "recv" : "sent";
...
static int log_recv_frame(cno_connection_t *conn, int *fd, cno_frame_t *frame)
{
...
static int log_sent_frame(cno_connection_t *conn, int *fd, cno_frame_t *frame)
{
...
static int write_to_fd(cno_connection_t *conn, int *fd, const char *data, size_t length)
{
    size_t wrote = 0;
...
static int log_recv_message(cno_connection_t *conn, int *fd, size_t stream, cno_message_t *msg)
{
...
static int log_recv_message_data(cno_connection_t *conn, int *fd, size_t stream, const char *data, size_t length)
{
    if (length) {
...
static int log_recv_message_end(cno_connection_t *conn, int *fd, size_t stream, int disconnect)
{
    fprintf(stdout, "%d: recv end of message; stream %s\n", *fd, disconnect ? "closed" : "half-closed");
    return CNO_OK;
}
